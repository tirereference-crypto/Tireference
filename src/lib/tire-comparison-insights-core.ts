...
import { comparisonSlugFromSizes, orderComparisonSizes } from './comparison-url';
...
/**
 * Link-free comparison insights core (Layer 3 content, no navigation).
 *
 * Never imports tire-comparison-links — that breaks the historical
 * links ⇄ insights import cycle. Navigation fields are always empty here;
 * the thin tire-comparison-insights wrapper attaches popular/upgrade links.
 */
...
function buildSeoContent(
  seo: ReturnType<typeof buildComparisonSeoBundle>,
  sizeA: string,
  sizeB: string,
  analysis: EngineeringAnalysis,
): ComparisonSeoContent {
  ...
  return {
    title: `${seo.title} | Tire Reference`,
...
export function buildComparisonInsightsCore(
  sizeA: string,
  sizeB: string,
  comparison: TireComparison,
  specsA: TireSpecs,
  ...
  const seoBundle = buildComparisonSeoBundle(sizeA, sizeB, comparison, specsA, specsB);
  ...
  const draft: ComparisonInsights = {
    ...
    specRows,
    quickVerdict,
    pageIntro: { sentence: seoBundle.answer },
    ...
    // Navigation is attached by tire-comparison-insights.ts — never here.
    upgradePaths: null,
    popularComparisons: [],
    vehicleCompatibility: buildVehicleCompatibility(sizeA, sizeB),
    seo: buildSeoContent(seoBundle, sizeA, sizeB, engineeringAnalysis),
    engineeringAnalysis,
...
export function isComparisonPublishable(sizeA: string, sizeB: string): boolean {
  ...
  const key = comparisonSlugFromSizes(ordered.current, ordered.new);
  const cached = publishableComparisonCache.get(key);
  if (cached !== undefined) return cached;

  const specsA = getTireSpecs(ordered.current);
  const specsB = getTireSpecs(ordered.new);
  const comparison = compareTires(ordered.current, ordered.new, 60);
  const approved = buildComparisonInsightsCore(
    ordered.current,
    ordered.new,
    comparison,
    specsA,
